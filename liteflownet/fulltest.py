...
arguments_strPath = './'
# arguments_strMode = 'training'
arguments_strOutPath = './'

for strOption, strArgument in getopt.getopt(sys.argv[1:], '', [ strParameter[2:] + '=' for strParameter in sys.argv[1::2] ])[0]:
    if strOption == '--model' and strArgument != '': arguments_strModel = strArgument # which model to use
    if strOption == '--path' and strArgument != '': arguments_strPath = strArgument # which model to use
    # if strOption == '--out' and strArgument != '': arguments_strOut = strArgument # path to where the output should be stored
    # if strOption == '--mode' and strArgument != '': arguments_strMode = strArgument
    if strOption == '--out' and strArgument != '': arguments_strOutPath = strArgument
# end
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        self.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.load(__file__.replace('fulltest.py', 'network-' + arguments_strModel + '.pytorch')).items() })
    # end
...
def run(first_path, second_path):
    ...
    backwarp_tenGrid = {}
    tenFirst = torch.FloatTensor(numpy.array(PIL.Image.open(first_path))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
    tenSecond = torch.FloatTensor(numpy.array(PIL.Image.open(second_path))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
    tenOutput = estimate(tenFirst, tenSecond)

    return tenOutput.numpy().transpose(1, 2, 0)

if __name__ == '__main__':
    # if arguments_strMode is "training":
    for idx in range(200):
        print(idx)
        file_name_first = "{:06}_10.png".format(idx)
        file_name_second = "{:06}_10.png".format(idx)
        first_path = os.path.join(arguments_strPath, "image_2", file_name_first)
        second_path = os.path.join(arguments_strPath, "image_2", file_name_second)

        # run the network
        output = run(first_path, second_path)
        output_file_name = "{:06}_10.npy".format(idx)
        output_full_path = os.path.join(arguments_strOutPath, output_file_name)
        numpy.save(output_full_path, output)
