...
class PWCNetwork(torch.nn.Module):
    ...
    def __init__(self):
        super(PWCNetwork, self).__init__()
        ...
        #                       torch.load('network-' + model + '.pytorch').items()})
    ...
    def load_state(self, path):
        self.load_state_dict(
            {strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.load(path).items()})
...
if __name__ is "__main__":
    # Set parameters
    model_path = "network-default.pytorch"
    first = "/content/pytorch-pwc/training/image_2/000000_10.png"
    second = "/content/pytorch-pwc/training/image_2/000000_11.png"

    # first = "./images/first.png"
    # second = "./images/second.png"
    out = "./out.flo"

    netNetwork = PWCNetwork()
    netNetwork.load_state(model_path)
    tenFirst = torch.FloatTensor(
        np.array(PIL.Image.open(first))[:, :, ::-1].transpose(2, 0, 1).astype(np.float32) * (1.0 / 255.0))
    tenSecond = torch.FloatTensor(
        np.array(PIL.Image.open(second))[:, :, ::-1].transpose(2, 0, 1).astype(np.float32) * (1.0 / 255.0))

    tenOutput = estimate(netNetwork, tenFirst, tenSecond)

    objOutput = open(out, 'wb')
